...
export const limits = {
  pdf: {
    ...
    maxLinesPerPage: 30,
    /** «Делить длинные абзацы»: самая большая длина части в строках. */
    maxLinesPerUnit: 30,
  },
  ...
  /** Выдача порций (CLAUDE.md, разделы 4.1 и 5). */
  learning: {
    /** «Напомнить позже» — через столько минут. */
    remindLaterMin: 60,
    /** «Захватить больше» — не больше стольких расширений. */
    maxMarginSteps: 3,
    /** «Ещё выше» / «Ещё ниже» — не больше стольких единиц в каждую сторону. */
    maxExtraUnits: 5,
    /** «Пропустить…»: при большем числе единиц в порции предлагается только «всю порцию». */
    maxSkipChoices: 40,
  },
} as const;
